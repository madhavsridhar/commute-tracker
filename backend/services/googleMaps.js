...
export async function fetchCommuteTime(source, destination) {
  ...
  try {
    ...
    const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${source}&destinations=${destination}&departure_time=now&key=${process.env.GOOGLE_MAPS_API_KEY}`;
